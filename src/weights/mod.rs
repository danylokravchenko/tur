pub mod downloader;
pub use downloader::{Downloader, ModelPaths};

pub mod gguf_varbuilder;
pub use gguf_varbuilder::VarBuilder as QVarBuilder;
...
impl VarBuilderX<'_> {
    ...
    pub fn all_keys(&self) -> Option<Vec<String>> {
        match &self.0 {
            Either::Left(_vb) => None, // SafeTensors VarBuilder doesn't expose all keys
            Either::Right(vb) => Some(vb.all_keys()),
        }
    }
}
