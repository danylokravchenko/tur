...
pub fn flash_attn_varlen_cpu(
    ...
    if !q.device().is_cpu() {
        candle_core::bail!("flash_attn_varlen_cpu is CPU only");
    }
    ...
    if dt != DType::F32 && dt != DType::F16 {
        candle_core::bail!("flash_attn_varlen_cpu supports f32 or f16 only");
    }
    ...
    if dk != d || dv != d {
        candle_core::bail!("head dim mismatch");
    }

    if hq % hk != 0 {
        candle_core::bail!("Invalid GQA: Hq={} not divisible by Hk={}", hq, hk);
    }
    if hq % hv != 0 {
        candle_core::bail!("Invalid GQA: Hq={} not divisible by Hv={}", hq, hv);
    }
    let rk = hq / hk;
    let rv = hq / hv;
    ...
    if seqlens_k_vec.len() != bsz {
        candle_core::bail!("seqlens_q and seqlens_k batch mismatch");
    }
    ...
    if causal {
        for i in 0..bsz {
            ...
            if lk < lq {
                candle_core::bail!("causal requires lk>=lq, got lk={lk} lq={lq} at batch {i}");
            }
        }
    }
    ...
    if cumsum_q[bsz] != total_q || cumsum_k[bsz] != total_k {
        candle_core::bail!("total_q/total_k mismatch with seqlens");
    }

    let slopes: Option<Vec<f32>> = if let Some(s) = alibi_slopes {
        let v = s.to_vec1::<f32>()?;
        if v.len() != hq {
            candle_core::bail!("alibi_slopes len {} != Hq {}", v.len(), hq);
        }
        Some(v)
    } else {
        None
